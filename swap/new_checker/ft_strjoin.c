...
/*   Updated: 2022/12/31 14:33:45 by hyounsi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	ft_strlens(char const *p)
{
	size_t	size;
...
char	*ft_strjoins(char const *s1, char const *s2, int *t)
{
	size_t	len_s1;
	size_t	len_s2;
	size_t	i;
...
char	*join_argv(char **argv, int *p)
{
	...
	{
		tmp = array;
		array = ft_strjoins(tmp, argv[i], p);
		free(tmp);
		i++;
	}
	return (array);
